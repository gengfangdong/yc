...
<html>
	...
	<body>
										...
										<tr>
											...
											<td class="rightTd" colspan="2">
												<div class="layui-upload-list">
												 ...
												 <button type="button" class="layui-btn layui-btn-normal" id="testList" style="float:right;">选择多文件</button>
											</td>
										</tr>
									</tbody>
								</table>
		    ...
		    ,choose: function(obj){   
		      ...
		      obj.preview(function(index, file, result){
		    	  var filenamea = file.name;
			      	for(var o = 0 ;o<filea.length;o++){
			      		if(filea[o].oldfilename == filenamea){
			      			layer.alert("文件已存在!");
			      			//break;
			      			return;
			      		}
		        ...
		        var tr = $(['<tr id="upload-'+ index +'">'
		          ...
		          ,'<td>'
		            ...
		            ,'<button class="layui-btn layui-btn-xs demo-delete" style="background:#1e9fff;width:54px;margin-left:0px;">删除</button>'
		          ,'</td>'
		        ,'</tr>'].join(''));
				...
				success:function(data){
					if(data.success == true){
						...
						if(data.data.figClass.figClass_caogery==0){
							...
							window.freeco_day = data.data.figClass.freeco_day;
						...
						//文件回显start
						var memotr = "";
    					filea = data.data.figClass.figFiles;
						//window.fileList = files;//定义全局变量
    					for(var i = 0 ;i<filea.length;i++){
    						memotr += '<tr id="upload-'+ i +'">'+
					          '<td>'+ filea[i].oldfilename +'</td>'+
					          '<td>'+
					          '<button class="layui-btn layui-btn-xs demo-delete" style="background:#1e9fff;" ><a href="<%=request.getContextPath()%>/FigClass/download/'+filea[i].newfilename+' " class="hoverColor" id="downLoad">下载</a></button>'+
					          '</td>'+
					        '</tr>';
    					}
		...
		function branchSub(){
			...
			if(filelist.length == 0&&filea.length == 0){
				alert("请选择资料上传!");
				return;
			}  
			var fd = new FormData();
			...
			for(var ttt = 0;ttt<filea.length;ttt++){
				fd.append('oldfilename',filea[ttt].newfilename);
			}
			$.ajax({
