	...
	<body>
		...
		layui.use('upload', function(){
			  ...
			  var uploadInst = upload.render({
			    elem: '#test1' ,
			    url: '<%=request.getContextPath()%>/About/updateAboutandFile',
			    auto:false,
			    bindAction: '#newsSave',
			    /*data:{
			    ...
			    before:function(obj){
			    	...
			    	this.data={
			    	"About_Title": document.getElementById("newsTitle").value,
			    	"About_Introduction":document.getElementById("newsAbstract").value,
					"About_KeyWords":document.getElementById("newsKeyWords").value,
					"About_Release_time":document.getElementById("newsDate").value,
					"About_status":about_status,
					"About_context":CKEDITOR.instances.newsDescription.getData(),
					"filename":document.getElementById("filename").value,
					"About_id":document.getElementById("newsid").value
			    	}//携带额外的数据
				    obj.preview(function(index, file, result){
				        $('#demo1').attr('src', result); //图片链接（base64）
				    });
		...
		<script>
			function save(){
				...
				if(oldfilename !=""){
					var about_title_page = oldfilename;
			    	var about_context = CKEDITOR.instances.newsDescription.getData();
					var aboutTitle = document.getElementById("newsTitle").value;
					var aboutAbstract = document.getElementById("newsAbstract").value;
					var aboutKeyWords = document.getElementById("newsKeyWords").value;
					var aboutDate = document.getElementById("newsDate").value;
					var newsStatus = document.getElementsByName("newsStates");
					var About_id = document.getElementById("newsid").value;
					var about_status="";
					for(var i=0;i<newsStatus.length;i++){
						if(newsStatus[i].checked == true){
							if("发布"==newsStatus[i].value){
					...
					$.ajax({
						url : '<%=request.getContextPath()%>/About/updateAbout',
						type : 'post',
						dataType:"json",
						data:{
							About_Title:aboutTitle,
							About_Introduction:aboutAbstract,
							About_KeyWords:aboutKeyWords,
							About_Release_time:aboutDate,
							About_status:about_status,
							About_context:about_context,
							filename:about_title_page,
							About_id:About_id
						},
			 ...
			 window.onload = function(){
				...
				success : function(data) {
					if(data.success == true){
						...
						var about_title_page = data.data.about_title_page;
						
						newsid.value=about_id;//新闻id
						...
						filename.value=about_title_page;
